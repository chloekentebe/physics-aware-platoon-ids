...
% Amplitude:       [0.08, 0.13]  — low gentle acceleration
ampValues      = 0.08  + (0.13  - 0.08)  * lhs(:,1);

% Velocity base:   [19.0, 22.0]  — ~20.7 m/s cruise
velBases       = 19.0  + (22.0  - 19.0)  * lhs(:,2);

% Spacing base:    [22.0, 28.0]  — wide gaps, conservative
spacingBases   = 22.0  + (28.0  - 22.0)  * lhs(:,3);

% deltaSpeed messages
deltaSpeedUp1  =  0.05 + (0.15  - 0.05)  * lhs(:,4);   % [0.05, 0.15]
deltaSpeedUp2  =  0.03 + (0.10  - 0.03)  * lhs(:,5);   % [0.03, 0.10]
deltaSpeedDown = -0.15 + (-0.05 - (-0.15)) * lhs(:,6); % [-0.15, -0.05]

% Timing offsets +-2s around baselines
...
% Output folder
outPath = '/Users/chloekentebe/physics-aware-platoon-ids/simulation/matlab/conservative_datav2';
...
for loopIdx = 1:numRuns
    ...
    accelerationProfile.Amplitude  = [ampValues(runIdx), 0, -0.08, 0];
    accelerationProfile.Period     = 60;
    accelerationProfile.PulseWidth = [8, 20, 8, 24];
    accelerationProfile.PhaseDelay = [5, 13, 30, 38];
    ...
    platoonFile = fullfile( ...
        outPath, ...
        sprintf('v2_platoon_run_%d.csv', loopIdx));
    ...
    % SAVE BSM TABLE
    % =====================================================
    bsmFile = fullfile(outPath, sprintf('v2_normal_conservative_bsm_run_%d.csv', loopIdx));
