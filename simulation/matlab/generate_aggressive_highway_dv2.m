...
% Amplitude:       [0.40, 0.50] — hard acceleration
ampValues = 0.40 + (0.50 - 0.50) * lhs(:,1);

% Velocity base:   [22.0, 23.0]
velBases = 22.0 + (23.0 - 22.0) * lhs(:,2);

% Spacing base:    [8.0, 12.0]  — tight gaps, industry 10m target
spacingBases   =  8.0 + (12.0 -  8.0) * lhs(:,3);

% deltaSpeed — more aggressive corrections
deltaSpeedUp1  =  0.20 + (0.35 - 0.20) * lhs(:,4);   % [0.20, 0.35]
deltaSpeedUp2  =  0.15 + (0.25 - 0.15) * lhs(:,5);   % [0.15, 0.25]
...
% Output folder
outPath = '/Users/chloekentebe/physics-aware-platoon-ids/simulation/matlab/aggressive_datav2';
...
for loopIdx = 1:numRuns
    ...
    accelerationProfile.Amplitude  = [ampValues(runIdx), 0, -1.0, 0];
    accelerationProfile.Period     = 60;
    accelerationProfile.PulseWidth = [6,  10,  5, 40];
    accelerationProfile.PhaseDelay = [1,  7,  20, 25];
    ...
    platoonFile = fullfile( ...
        outPath, ...
        sprintf('v2_platoon_run_%d.csv', loopIdx + 50));
    ...
    % SAVE BSM TABLE
    % =====================================================
    bsmFile = fullfile(outPath, sprintf('v2_normal_aggressive_bsm_run_%d.csv', loopIdx));
