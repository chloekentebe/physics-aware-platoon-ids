function [initialActorPose,accelerationProfile,stopTime] = ...
    congested_highway_scenario( ...
    tractorTrailerParameters,vehicleDimension, currentLHS)

% =========================================================
% CONGESTED HIGHWAY PROFILE
%
% Target:
% Cruise Speed      ≈ 19 m/s
% Initial Spacing   ≈ 15 m
% Steady Spacing    ≈ 8 m
% =========================================================
...
% Gentle acceleration
ampValues = 0.07 + (0.12 - 0.07)*currentLHS(1,1);

% Cruise speed
velBases = 18.5 + (19.5 - 18.5)*currentLHS(1,2);

% Initial spacing
spacingBases = 12.0 + (16.0 - 12.0)*currentLHS(1,3);

% RSU guidance values

deltaSpeedUp1  =  0.1 + (0.15 - 0.1)*currentLHS(1,4);
deltaSpeedUp2  =  0.05 + (0.09 - 0.05)*currentLHS(1,5);
deltaSpeedDown = -0.12 + (-0.05 - (-0.12))*currentLHS(1,6);
...
velDeltas = [0.0 -0.05 -0.1 -0.15 -0.2];
...
accelerationProfile.Amplitude = [ampValues 0 -0.3 0];
accelerationProfile.Period = 60;
accelerationProfile.PulseWidth = [7 15 8 30];
accelerationProfile.PhaseDelay = [1 8 23 31];
