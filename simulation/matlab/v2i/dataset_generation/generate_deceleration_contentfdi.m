...
% Content FDI — wrong deltaSpeed at correct timing
% No timing shifts, no attack window — whole sim affected
% deltaSpeed ranges chosen to be subtle but wrong direction
attackDeltaSpeed1 = -0.20 + (0.20 - (-0.20)) * lhs(:, 10);  % [-0.20, +0.20]
attackDeltaSpeed2 = -0.20 + (0.20 - (-0.20)) * lhs(:, 11);  % [-0.20, +0.20]
attackDeltaSpeed3 = -0.20 + (0.20 - (-0.20)) * lhs(:, 12);  % [-0.20, +0.20]
...
for loopIdx = 1:numRuns
    ...
    scenarioHandle = @(p1, p2) deceleration_highway_scenario(p1, p2, currentLHS);
    ...
    assignin('base', 'attackType',       'ContentFDI');
    assignin('base', 'runattackDeltaSpeed1', round(attackDeltaSpeed1(runIdx), 2));
    assignin('base', 'runattackDeltaSpeed2', round(attackDeltaSpeed2(runIdx), 2));
    assignin('base', 'runattackDeltaSpeed3', round(attackDeltaSpeed3(runIdx), 2));

    % Create timeseries for attack params so Simulink can read them
    t  = (0:0.1:60)';
    Ns = length(t);

    assignin('base', 'attackTypeNum',  ones(Ns,1) * 6);   % 6=ContentFDI
    ...
    T.Profile      = repmat("Deceleration_Highway", N, 1);
    ...
    T.AttackType     = repmat("ContentFDI", N, 1);
    T.AttackerID     = repmat(-1,           N, 1);
    T.AttackMag      = repmat(0,            N, 1);
    T.AttackStart    = repmat(0,            N, 1);
    T.AttackDuration = repmat(60,           N, 1);
    T.AttackActive   = ones(N, 1);
    T.TimingShift1   = repmat(0,            N, 1);  % timing unchanged
    T.TimingShift2   = repmat(0,            N, 1);
    T.TimingShift3   = repmat(0,            N, 1);
    T.AttackDelta1   = repmat(attackDeltaSpeed1(runIdx), N, 1);
    T.AttackDelta2   = repmat(attackDeltaSpeed2(runIdx), N, 1);
    T.AttackDelta3   = repmat(attackDeltaSpeed3(runIdx), N, 1);

    % =====================================================
    % SAVE PLATOON TABLE
    % =====================================================
    platoonFile = fullfile(outPath, sprintf('platoon_contentfdi_deceleration_%d.csv', loopIdx));
    ...
    for b = 1:length(bsmSignals)
        ...
        try
            ...
            for slot = 1:5
                ...
                tempTable.Profile      = repmat("Deceleration_Highway",   M, 1);
                ...
                tempTable.AttackType     = repmat("ContentFDI", M, 1);
                tempTable.AttackerID     = repmat(-1,           M, 1);
                tempTable.AttackMag      = repmat(0,            M, 1);
                tempTable.AttackStart    = repmat(0,            M, 1);
                tempTable.AttackDuration = repmat(60,           M, 1);
                tempTable.AttackActive   = ones(M, 1);
                tempTable.TimingShift1   = repmat(0,            M, 1);
                tempTable.TimingShift2   = repmat(0,            M, 1);
                tempTable.TimingShift3   = repmat(0,            M, 1);
                tempTable.AttackDelta1   = repmat(attackDeltaSpeed1(runIdx), M, 1);
                tempTable.AttackDelta2   = repmat(attackDeltaSpeed2(runIdx), M, 1);
                tempTable.AttackDelta3   = repmat(attackDeltaSpeed3(runIdx), M, 1);
                tempTable.IsSenderAttacker = zeros(M, 1);   % RSU not a BSM sender
    ...
    bsmFile = fullfile(outPath, sprintf('bsm_contentfdi_deceleration_%d.csv', loopIdx));
