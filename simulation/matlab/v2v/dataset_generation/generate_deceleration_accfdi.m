...
% Attack parameters
attackMagnitudes = 0.05 + (0.20 - 0.05) * lhs(:, 10);
...
for loopIdx = 1:numRuns
    ...
    attackTypeMap = containers.Map(...
        ...
        {0, 1, 2, 3, 4});
    attackTypeNum = attackTypeMap('AccFDI');
    ...
    T.AttackType     = repmat("AccFDI",                     N, 1);
    ...
    % SAVE PLATOON TABLE
    % =====================================================
    platoonFile = fullfile(outPath, sprintf('platoon_accfdi_deceleration_%d.csv', loopIdx));
    ...
    for b = 1:length(bsmSignals)
        ...
        try
            ...
            for slot = 1:5
                ...
                tempTable.AttackType   = repmat("AccFDI",                 M, 1);
    ...
    bsmFile = fullfile(outPath, sprintf('bsm_accfdi_deceleration_%d.csv', loopIdx));
