...
% Attack parameters
attackMagnitudes = 0.5  + (2.0  - 0.5)  * lhs(:, 10);  
...
outPath = '/Users/chloekentebe/physics-aware-platoon-ids/simulation/matlab/attack_dataset';

for loopIdx = 1:numRuns
    runIdx = runOrder(loopIdx);
    currentLHS = lhs(runIdx, :);

    % Pass attack params to workspace BEFORE setup
    assignin('base', 'attackType',      'SpeedFDI');
    assignin('base', 'attackerVehicle', attackerVehicles(runIdx));
    assignin('base', 'attackMagnitude', attackMagnitudes(runIdx));
    assignin('base', 'attackStart',     attackStarts(runIdx));
    assignin('base', 'attackDuration',  attackDurations(runIdx));
    ...
    T.AttackType     = repmat("SpeedFDI",                   N, 1);
    ...
    % SAVE PLATOON TABLE
    % =====================================================
    platoonFile = fullfile(outPath, sprintf('platoon_speedfdi_congested_%d.csv', loopIdx));
    ...
    for b = 1:length(bsmSignals)
        ...
        try
            ...
            for slot = 1:5
                ...
                tempTable.AttackType   = repmat("SpeedFDI",               M, 1);
    ...
    bsmFile = fullfile(outPath, sprintf('bsm_speedfdi_congested_%d.csv', loopIdx));
